import { buildRenderUnits, renderUnitKey } from './messageGrouping';
...
function makeImage(id: string, overrides: Partial<Message> = {}): Message {
  return {
    ...
    sender: 'alice',
    senderName: 'alice',
    // Empty content is the exact "no caption" signal ConversationScreen's send flow already
    // sends for an uncaptioned image — no real caption, by default.
    content: '',
    messageType: 'IMAGE',
...
describe('buildRenderUnits', () => {
  it('groups two or more consecutive same-sender images within the window into one imageGroup unit', () => {
    ...
    expect(units[0].type).toBe('imageGroup');
  ...
  it('does not group a single lone image — stays a single unit', () => {
    ...
    expect(buildRenderUnits(messages)).toEqual([{ type: 'single', message: messages[0] }]);
  });
  ...
  it('breaks the group when a different sender sends an image in between', () => {
    ...
    expect(buildRenderUnits(messages)).toEqual([
  ...
  it('breaks the group across a day boundary', () => {
    ...
    // local calendar-day boundary regardless of the test runner's timezone.
    const messages = [
  ...
  it('breaks the group at a reply or forwarded image', () => {
    ...
    expect(buildRenderUnits(messages).map((u) => u.type)).toEqual(['single', 'single', 'single', 'single', 'single']);
  });
  ...
  it('keeps text messages before and after a group as separate single units', () => {
    ...
    expect(buildRenderUnits(messages).map((u) => u.type)).toEqual(['single', 'imageGroup', 'single']);
  });
});

describe('renderUnitKey', () => {
  it('uses the message id for a single unit', () => {
    expect(renderUnitKey({ type: 'single', message: makeText('1') })).toBe('1');
  });

  it('uses a group-prefixed key based on the first message for an imageGroup unit', () => {
    const messages = [makeImage('1'), makeImage('2')];
    expect(renderUnitKey({ type: 'imageGroup', messages })).toBe('group-1');
  });
});
