...
export default function ShortcutsPanel() {
  return (
    <div>
      ...
      <p className="mt-0.5 text-sm text-gray-500">Speed up the things you do most.</p>
          ...
          <li key={s.action} className="flex items-center justify-between gap-4 px-4 py-3">
            ...
            <span className="flex flex-shrink-0 items-center gap-1">
              {s.keys.map((k) => (
                <kbd
                  key={k}
                  className="rounded-md border border-gray-200 bg-gray-50 px-2 py-0.5 text-xs font-medium text-gray-600 shadow-sm"
                >
                  {k}
                </kbd>
              ))}
