...
export default function GlobalSearchModal({ open, onClose, onNavigate, currentUsername, rooms, conversations, userCache }: GlobalSearchModalProps) {
  ...
  const getRoomLabel = useCallback((roomId: string): string => {
    if (roomId.startsWith('dm:')) {
      const conv = conversations.find((c) => c.id === roomId.slice(3))
      const other = conv?.participants.find((p) => p !== currentUsername)
      const u = other ? userCache[other] : undefined
      return u?.displayName || (u?.uniqueHandle ? `@${u.uniqueHandle}` : other) || 'Direct Message'
    }
    const room = rooms.find((r) => r.roomId === roomId)
    return room ? `#${room.name}` : `#${roomId}`
  }, [rooms, conversations, currentUsername, userCache])
  ...
  const grouped = results.reduce<Record<string, Message[]>>((acc, msg) => {
  ...
  return (
    ...
    >
      ...
      {/* Panel */}
      <div
        className="relative w-full max-w-2xl bg-white dark:bg-[#1a242b] rounded-2xl shadow-2xl overflow-hidden max-h-[70vh] flex flex-col"
      ...
      >
        ...
        <div className="flex items-center gap-3 px-4 py-3 border-b border-gray-200 dark:border-gray-700">
          ...
          <input
            ...
            className="flex-1 text-sm text-gray-900 dark:text-gray-100 placeholder:text-gray-400 focus:outline-none bg-transparent"
            aria-label="Search messages"
            ...
            <div className="py-2">
              {Object.entries(grouped).map(([roomId, messages]) => (
                <div key={roomId} className="mb-2">
                  <div className="px-4 py-1.5 text-[10px] font-semibold text-gray-400 uppercase tracking-wider sticky top-0 bg-white/95 dark:bg-[#1a242b]/95 backdrop-blur-sm">
                    {getRoomLabel(roomId)}
                  </div>
                  {messages.map((msg) => (
                    <button
                      ...
                      className="w-full text-left px-4 py-2.5 hover:bg-gray-50 dark:hover:bg-[#202c33] transition-colors flex flex-col gap-0.5"
                      data-testid="search-result-item"
                    >
                      <div className="flex items-center justify-between gap-2">
                        <span className="text-xs font-semibold text-emerald-700 dark:text-emerald-400">{msg.senderName}</span>
