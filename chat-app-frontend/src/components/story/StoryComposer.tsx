...
export default function StoryComposer({ open, onClose }: StoryComposerProps) {
  ...
  const reset = () => {
    ...
    setImageFile(null); setImagePreview(null); setError(null); setPosting(false)
  }
  const close = () => { reset(); onClose() }

  // Selecting a photo shows a PREVIEW with a caption box — it is NOT posted until
  // the user taps Share (WhatsApp/Instagram style).
  ...
  const handleShare = async () => {
    ...
    if (mode === 'image') {
      ...
      setPosting(true)
      try {
        const { url } = await uploadApi.uploadFile(imageFile)
  ...
  const canShare = mode === 'image' ? !!imageFile : !!text.trim()
      ...
      <div className="w-full max-w-sm overflow-hidden rounded-2xl bg-white shadow-2xl dark:bg-[#1a242b]">
        <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100 dark:border-gray-800">
          <h3 className="font-semibold text-gray-900 dark:text-gray-100">
            {mode === 'image' ? 'Share photo' : 'Add to your story'}
          </h3>
